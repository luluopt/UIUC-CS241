...
int read_1_virtual_address(mmu *test_mmu) {
  uintptr_t virtual_address = PAGE_SIZE;
  char buff[PAGE_SIZE];
...
int read_and_write_1_virtual_address(mmu *test_mmu) {
  ...
  mmu_write_to_virtual_address(test_mmu, virtual_address, TEST_PID, test_string,
...
int interleave(mmu *test_mmu) {
  uintptr_t virtual_address1 = PAGE_SIZE;
  uintptr_t virtual_address2 = PAGE_SIZE * 2;
  ...
  assert(test_mmu->num_page_faults == 0);
  assert(test_mmu->num_segmentation_faults == 0);
  // we still need to traverse the table 2 times, since the base address differ.
...
int multiprocess(mmu *test_mmu) {
  uintptr_t virtual_address = PAGE_SIZE * 2;
  size_t pid1 = 1234;
  size_t pid2 = 9001;
...
int same_vpns_diff_offset(mmu *test_mmu) {
  ...
  for (size_t offset = 0; offset < PAGE_SIZE; offset++) {
    ...
    uintptr_t virtual_address_increment_offset = virtual_address + offset;
...
int segfault(mmu *test_mmu) {
  ...
  mmu_write_to_virtual_address(test_mmu, (uintptr_t)NULL, TEST_PID, test_string,
                               strlen(test_string) + 1);
