 ...
 * target ("") whose neighbors are the build targets specified by the
 * `targets` argument. That is to say, `graph_neighbors(dep_graph, "")`
 * will return a vector containing the strings found in `targets`.
 *
 * The edges in this graph represent dependencies between rules:
 * `graph_adjacent(dep_graph, "tgt1", "tgt2")` implies that "tgt1"
 * depends on "tgt2". To find all neighbors of a vertex in a vector, use
 * the `graph_neighbors(dep_graph, "tgt")` method.
 *
 ...
 * will be used as the only build target (This is consistent with GNU Make).
 * So in this case:
 *
 * ''  : {'a'}
 * 'a' : {'c', 'b'}
