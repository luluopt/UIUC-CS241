 ...
 */
FILE *open_makefile(const char *makefile_name, char **run_targets) {
    // Open the makefile
    if (!makefile_name && !run_targets) {
...
graph *parser_parse_makefile(const char *makeFileName, char **run_targets) {
    FILE *f = open_makefile(makeFileName, run_targets);
    ...
    for (char **targetPtr = run_targets; *targetPtr; ++targetPtr) {
        if (!graph_contains_vertex(dependency_graph, *targetPtr)) {
            ...
            graph_set_edge_value(dependency_graph, "", *targetPtr,
                                 (void *)(targetPtr - run_targets));
        }
    }
#ifdef EASY_MODE
    remove_unnecessary_targets(dependency_graph, run_targets);
#endif
