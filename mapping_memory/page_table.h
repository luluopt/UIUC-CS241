...
/*
 * The following structs are bit fields (
 * http://www.tutorialspoint.com/cprogramming/c_bit_fields.htm )
 * for the standard 32bit virtual and physical address, 4kb offset, 2 level page
 * table.
 */
...
typedef struct {
  page_directory_entry entries[PAGE_SIZE / sizeof(page_directory_entry)];
} page_directory;
...
/* However for this lab you will only need to fiddle with:
 ...
 *
 * If the processor generates a page-fault exception, the operating system
 * generally needs to carry out the following operations:
...
/*
 * Read/write (R/W) flag, bit 1 -
 * Specifies the read-write privileges for a page or group of pages (in the case
 * of a page-directory entry that points to a page table).
 * When this flag is clear, the page is read only; when the flag is set, the
 * page can be read and written into.
 */

/*
 * User/supervisor (U/S) flag, bit 2 -
 * Specifies the user-supervisor privileges for a page or group of pages (in the
 * case of a page-directory entry that points to a page table).
 ...
 * Controls the write-through or write-back caching policy of individual pages
 * or
 * page tables. When the PWT flag is set, write-through caching is enabled for
 * the
 ...
 * set, caching of the associated page or page table is prevented; when the flag
 * is
 ...
 * page or page table is initially loaded into physical memory. The processor
 * then
 * sets this flag the first time a page or page table is accessed. This flag is
 * a
 * “sticky” flag, meaning that once set, the processor does not implicitly clear
 * it.
 * Only software can clear this flag.
 * The accessed and dirty flags are provided for use by memory management
 * software
...
/*
 * Dirty (D) flag, bit 6 -
 * Indicates whether a page has been written to when set. (This flag is not used
 * in
 ...
 * memory. The processor then sets this flag the first time a page is accessed
 * for
 * a write operation.
 * This flag is “sticky,” meaning that once set, the processor does not
 * implicitly
...
/* Page attribute table index (PAT) flag, bit 7 in page-table entries for
4-KByte pages and
...
that
support the page attribute table (PAT), this flag is used along with the PCD and
PWT flags to select an entry in the PAT, which in turn selects the memory type
for the page (see Section 10.12., “Page Attribute Table (PAT)”). For processors
that do not support the PAT, this bit is reserved and should be set to 0.*/
...
CR4 is set, the page-table or page-directory entry for the page is not
invalidated
...
Only software can set or clear this flag. For page-directory entries that point
to
page tables, this flag is ignored and the global characteristics of a page are
set
in the page-table entries. See Section 3.11., “Translation Lookaside Buffers
...
CR4 are set, the processor generates a page fault if reserved bits are not set
to 0.
For Pentium II and earlier processors. Bit 7 in a page-table entry is reserved
and
